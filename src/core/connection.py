import logging
logger = logging.getLogger(__name__)   # module-level, once, at top of file
...
class ConnectionService(QObject):
    ...
    ConnectionService is the only object the rest of the app should talk to.
    It never touches the serial port itself -- Connection (running on
    _connection_thread) does that. ConnectionService's job is to:
    ...
    All state changes MUST go through _transition() -- never assign
    self._state directly anywhere else in this class.
    """

    _TRANSITIONS: dict[ConnectionState, set[ConnectionState]] = {
        ConnectionState.DISCONNECTED: {ConnectionState.CONNECTING},
        ConnectionState.CONNECTING:   {ConnectionState.CONNECTED, ConnectionState.FAILED, ConnectionState.STOPPING},
        ConnectionState.CONNECTED:    {ConnectionState.STOPPING},
        ConnectionState.FAILED:       {ConnectionState.CONNECTING},
        ConnectionState.STOPPING:     {ConnectionState.DISCONNECTED, ConnectionState.FAILED},
    }

    state_changed = Signal(ConnectionState)
    """Emitted every time _transition() succeeds. Parent/UI should connect
    to this to react to state changes -- it's the only outward-facing
    notification this class makes about its own state."""


    close_connection = Signal()
    """Emitted to ask the worker (Connection, on _connection_thread) to
    close its port. Crosses a thread boundary via a queued connection, so
    this returns immediately -- it does NOT mean the port is closed yet.
    The actual confirmation comes back later via Connection.closed,
    handled in _on_connection_closed."""
    ...
    def _on_connection_result(self, success: bool):
        ...
        Runs on whichever thread emits the signal originally, but since
        this is a queued cross-thread connection, this method body itself
        executes back on ConnectionService's own thread.
        """
        if success:
            if not self._transition(ConnectionState.CONNECTED):
                self.close_connection.emit()
        else:
            if self._transition(ConnectionState.FAILED):
                self._teardown()

    def _on_connection_closed(self):
        """Handles Connection.closed -- confirmation the port is shut."""
        if self._transition(ConnectionState.DISCONNECTED):
            self._teardown()
    ...
    def _buildup(self):
        ...
        self.close_connection.connect(self._connection.close)
    ...
    def start(self):
        ...
        if self._transition(ConnectionState.CONNECTING):
            self._buildup()
        else:
            logger.warning("start() called from invalid state: %s", self._state)

    def stop(self):
        """Requests a stop, if currently CONNECTING or CONNECTED.
        No-ops otherwise."""
        if self._transition(ConnectionState.STOPPING):
            self.close_connection.emit()
